...
def run_polling():
    """Run bot in polling mode with improved reliability and auto-restart"""
    offset = 0
    ...
    logger.info("💡 Бот запущен в режиме 24/7. Для остановки нажмите Ctrl+C")
    logger.info("🔄 Включена защита от засыпания и автоматический перезапуск")
    ...
    while bot.running:
        try:
            ...
            consecutive_errors += 1
            if e.code == 401:
                logger.error("Неверный токен бота! Проверьте TELEGRAM_BOT_TOKEN в Secrets")
                break
            elif e.code == 409:
                logger.warning(f"Конфликт: другой экземпляр бота уже запущен")
                time.sleep(30)  # Wait longer for conflicts
            else:
        ...
        # Safety check for too many consecutive errors
        if consecutive_errors >= max_consecutive_errors:
            logger.error(f"Слишком много ошибок подряд ({consecutive_errors}). Перезапуск через 60 секунд...")
            time.sleep(60)
        ...
        if current_time - last_heartbeat > 30:
            try:
                # Send a simple API call to keep connection alive
                with urllib.request.urlopen(f"{bot.api_url}/getMe", timeout=10) as response:
                    heartbeat_data = json.loads(response.read().decode('utf-8'))
                if heartbeat_data.get('ok'):
                    logger.info(f"💓 Heartbeat OK - {time.strftime('%H:%M:%S')}")
...
if __name__ == "__main__":
    ...
    logger.info("🤖 Запуск экономического викторина-бота (24/7 режим)...")
    bot = TelegramBot(TOKEN)

    # Initialize keep-alive service
    keep_alive_service = KeepAlive(bot)
    ...
    try:
        ...
        if e.code == 401:
            logger.error("Неверный токен! Проверьте TELEGRAM_BOT_TOKEN в Secrets")
        elif e.code == 404:
            logger.error("Токен не найден! Убедитесь, что токен правильный")
        else:
    ...
    logger.info("🔄 Запуск в режиме непрерывной работы (24/7)...")
    logger.info("🛡️ Включена защита от сбоев и автоматический перезапуск")
    ...
    try:
        ...
        logger.info("🔄 Попытка автоматического перезапуска через 10 секунд...")
        time.sleep(10)
