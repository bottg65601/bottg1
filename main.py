...
# Quiz questions (сокращено для примера, в реальном коде используйте полный список)
quiz_questions = [
    {
...
async def handle_timeout(chat_id: int, bot):
    ...
    await bot.send_message(
        chat_id=chat_id,
        text=f"⏰ Время вышло! Правильный ответ: <b>{question['answer']}</b>",
        parse_mode="HTML"
    )
...
# Остальные функции (show_dictionary, show_course, show_help) остаются аналогичными
